    ...
    data_label = T, horiz = F, colour = "green") 
{
    ...
    chart <- ms_barchart(data = data %>% mutate(category_proxy = .data[[x]]), 
        x = x, y = y, group = "category_proxy", labels = data_label) %>% 
        chart_settings(vary_colors = TRUE, grouping = "standard", 
            overlap = 90, gap_width = 150, dir = horizontal) %>% 
